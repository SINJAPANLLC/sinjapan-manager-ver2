...
export function AiPage() {
  ...
  const handleListGenerate = async () => {
    if (!listTopic.trim() || !searchLocation.trim() || isLoading) return;
    setIsLoading(true);
    setGeneratedList('');
    setGeneratedLeads([]);
      ...
      <div className="bg-white rounded-2xl shadow-soft border border-slate-100 p-6">
        ...
        {activeTab === 'list' && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-500">Googleマップから実際のビジネス情報（店舗名、住所、電話番号など）を取得します</p>

            <div className="grid md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">業種・キーワード *</label>
                <input
                  type="text"
                  value={listTopic}
                  onChange={(e) => setListTopic(e.target.value)}
                  placeholder="例: 軽貨物、美容院、飲食店"
                  className="input-field"
                />
              </div>
              <div>
            ...
            <button
              ...
              disabled={isLoading || !listTopic.trim() || !searchLocation.trim()}
              className="btn-primary flex items-center gap-2"
            >
